...
% %     filename = [slot_fn,int2str((i-1)/20),'.csv'];
% %     if (isfile(filename))
% %         slot = load(filename);
% %         for j = 1 : length(slot)
% %             s(j) = rectangle('Position',[slot(j,1) - car_len/2, ...
% %             slot(j,2) - car_wid/2, car_len, car_wid], ...
% %             'FaceColor', 'g');
% %             hold on;
% %         end
% %     end
...
% color map
ego_cm = colormap(autumn(numberOfFrames));
for i = 1 : 5 : numberOfFrames
    % plot ego vehicle
    ego_x = traj(i, 1);
    ego_y = traj(i, 2);
    ego_theta = traj(i, 3);
...
alpha(0.3)
c = colorbar();
c.Label.String = 'Normalized Time Scale';
